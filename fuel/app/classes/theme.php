...
class Theme extends Fuel\Core\Theme
{
	...
	/**
	 *
	 * Updated 15.12.2012 by Marcus Reinhardt
	 * - Added Modules path to get a better structure in the theme folder
	 *
	...
	protected function find_file($view, $themes = null)
	{
		...
		foreach ($themes as $theme)
		{
			...
			{
				if (is_file($path = $theme['path'].'/modules/'.$path_prefix.$file.$ext))
				{
					return $path;
				}
				...
				elseif (is_file($path = $theme['path'].'modules/'.$file.$ext))
				{
					return $path;
				}
